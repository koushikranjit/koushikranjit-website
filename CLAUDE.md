...
### 2026-08-18 (Session 7) — New `/resume` page from uploaded résumé PDF
- **User uploaded `Koushik_Ranjit_Prop_Firm_Operations_Resume.pdf`** and asked for a profile page at `koushikranjit.in/resume`. This is a professional résumé (Client Support & Discord Moderator role at Titan Capital Markets, prior role at AlphaCapitalGroup), not a trading-strategy PDF — no copyright concerns, it's the user's own résumé.
- **New page: `/resume`** (`src/app/resume/page.tsx`) — matches the blog/riskandearning dark + emerald glass-card design system (same nav/footer pattern, DM Sans). Sections: header with avatar initials + contact row (phone, email, location, LinkedIn), Summary, Skills (as pill tags), Experience (2 roles with full bullet lists), a two-column Education + Languages row, Certifications & Licenses, and a closing "Let's connect" CTA.
- **Note: the résumé lists a different phone number** (`+91 95646 59426`) than the one used elsewhere on the site (`+91 95477 74580`, the WhatsApp/business number). Used the résumé's own listed number verbatim on this page specifically, since it's professional/job-application contact info scoped to this page — did not touch the number used elsewhere on the site.
- **Source PDF hosted as-is** at `public/downloads/koushik-ranjit-resume.pdf`, linked via "Download Résumé (PDF)" buttons (hero + bottom CTA) — same download pattern as the blog posts, fine here since it's the user's own document.
- **Homepage footer** (`public/index.html`) — added a "Résumé" link to the footer Links column, between Blog and Risk Disclaimer. Did **not** add it to the main scroll-nav (already has 8 items: About/Experience/Community/Recognition/Media/Book/Gallery/Blog) — footer + direct URL access seemed sufficient without further crowding it.
- **`sitemap.xml`** — added `/resume`.
- Verified with `npm run build` (0 errors, `/resume` route present) + `npm run start` + `curl` (PDF download 200, footer link present in homepage HTML) + Playwright screenshots (desktop/mobile) — page renders cleanly, two-column Education/Languages grid collapses correctly on mobile, no real console errors beyond the same pre-existing sandbox-blocked Google Fonts request seen on every page. `node_modules` + `.next` deleted after, per storage rules.
