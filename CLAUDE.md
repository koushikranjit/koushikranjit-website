...
### 2026-08-11 (Session 4) — Custom Discord bot built from scratch (`discord-bot/`), branch `claude/custom-discord-bot-xic3a5`
- **New standalone Node.js bot, not part of the Next.js site.** Discord bots need a persistent gateway connection, which doesn't run on Vercel serverless functions — lives in its own `discord-bot/` folder in this repo but needs a separate always-on host (Railway/Render/VPS) to actually run. User had no hosting preference; documented both options in `discord-bot/README.md` rather than picking one and standing it up, since no Discord token/server IDs/hosting account were available this session to actually deploy it.
- **Scope, from the user:** "all type of work" for a **P2P trading server** — specifically daily buy/sell price updates, an auto message when a deal completes, and a ticket system, plus general-purpose moderation/utility commands.
- **Embed design matched to a reference screenshot the user provided** (another server's "Vault APP" bot): a bold BUY/SELL price card (`🟢 BUY PRICE` / `🔴 SELL PRICE` in a currency-formatted code block, `₹` default) for the daily rate post, and a **"✅ Secure Trade Completed"** card for the public deal log that deliberately omits buyer/seller identity — only trade type + volume — matching the privacy pattern in the reference screenshot's `#completed-transactions` channel. Branding (`BRAND_NAME`, `CURRENCY_LABEL`, `CURRENCY_SYMBOL`) is configurable via env vars rather than hardcoded to any one name.
- **User asked whether an existing public bot could do this instead of building custom** — answered no: the reference bot itself is custom-built (not a public template), and no off-the-shelf bot (MEE6/Dyno-style mod bots, generic escrow bots) replicates this exact branded price-ticker + trade-completion-log combo. Proceeded with the custom build already in progress.
- **Architecture:** discord.js v14, modular slash commands (`src/commands/<category>/*.js`, auto-loaded), events (`src/events/*.js`, auto-loaded), flat-JSON storage (`src/data/store.json`, gitignored — fine at this scale, flagged in the README as swappable for a real DB later). Dropped `node-cron` in favor of a ~20-line dependency-free daily-fire-once scheduler (`src/utils/scheduler.js`) after `node-cron`'s `uuid` transitive dep flagged a moderate `npm audit` vulnerability — `npm audit` is clean (0 vulnerabilities) after the swap.
- **Commands:** `/price show|set`, `/offer` (member posts a buy/sell offer), `/deal start` (staff opens a private buyer+seller channel with Mark Complete/Cancel buttons), `/ticket-panel` (posts an Open Ticket button → private channel), `/kick` `/ban` `/timeout` `/warn` `/warnings` `/clear`, `/ping` `/userinfo` `/serverinfo` `/avatar` `/poll` `/announce`, `/help`. All permission-gated appropriately (Discord default member permissions where the whole command is restricted; manual `interaction.memberPermissions` checks where only a subcommand/button action needs to be, e.g. `/price set` vs `/price show`).
- **Verified before pushing:** `npm install` clean, `npm audit` → 0 vulnerabilities, `node --check` on every source file, and a smoke test importing the command/event loaders confirmed all 17 commands and all 3 event files resolve without errors. Did **not** invite the bot to a real server or run it live — no bot token/`CLIENT_ID`/channel IDs were provided this session; `.env.example` documents every variable needed, `README.md` covers Developer Portal setup, local run, and both hosting paths (Docker for Railway/Render with a note to mount a persistent volume at `/app/src/data`, or a plain VPS + pm2/systemd).
- **Not yet done, needs the user:** create the bot application in the Discord Developer Portal, invite it to the actual P2P server, provide channel/role IDs, pick and stand up an always-on host, run `npm run deploy-commands` once live. `node_modules` deleted after testing, per storage rules.
