...
const EA_PLAN_ID = process.env.RAZORPAY_EA_PLAN_ID || 'plan_TExvqw70wzSikC'
...
export async function GET(req: Request) {
  try {
    ...
    const res = await fetch(`https://api.razorpay.com/v1/subscriptions?count=100&plan_id=${EA_PLAN_ID}`, {
    ...
    const matched = (data.items || [])
      ...
      .map((sub: { id: string; status: string; created_at: number }) => ({
        id: sub.id,
        status: sub.status,
        plan: 'KR Auto Trading (XAU/USD EA)',
        created: new Date(sub.created_at * 1000).toLocaleDateString('en-IN'),
      }))

    if (matched.length === 0) {
      return NextResponse.json({ error: 'No active EA trading subscriptions found for this email.' }, { status: 404 })
    }
